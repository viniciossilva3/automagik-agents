...
class NotionTools:
    def __init__(self):
        ...
        self.__notion__ = self.__initialize_notion_client()

        # Initialize tools for different categories
        self.__init_database_tools()
        self.__init_page_tools()
        self.__init_block_tools()
    ...
    def create_database(
        ...
        return self.__notion__.databases.create(
            parent=parent, title=title, properties=properties, icon=icon, cover=cover
        )
    ...
    def create_database_item(
        self, ctx: RunContext[Dict], database_id: str, properties: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def update_database_item(
        self, ctx: RunContext[Dict], page_id: str, properties: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        """
        return self.__notion__.pages.update(page_id=page_id, properties=properties)
    ...
    def get_page_property(
        self, ctx: RunContext[Dict], page_id: str, property_id: str
    ) -> Dict[str, Any]:
        """
    ...
    def update_block(
        self, ctx: RunContext[Dict], block_id: str, **properties
    ) -> Dict[str, Any]:
        """
        Updates a block.
    ...
    def get_block_children(
        self, ctx: RunContext[Dict], block_id: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        return self.__notion__.blocks.children.list(block_id=block_id).get(
            "results", []
        )

    def append_block_children(
        self, ctx: RunContext[Dict], block_id: str, children: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
    ...
    def __initialize_notion_client(self) -> Client:
        """Initialize Notion client and handle exceptions."""
        try:
            return Client(auth=self.get_notion_token())
        except Exception as e:
            raise ConnectionError(f"Failed to initialize Notion client: {str(e)}")

    def __init_database_tools(self):
        """Initialize database related tools."""
        self.__tools__ = [
    ...
    def __init_page_tools(self):
        """Initialize page related tools."""
    ...
    def __init_block_tools(self):
        """Initialize block related tools."""
        self.__tools__.extend(
            [
                self.get_block,
                self.update_block,
